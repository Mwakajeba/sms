...
<body>
    <div class="header">
        ...
        @endif
        <h1>LOAN AGING REPORT</h1>
        <p style="margin: 0; font-size: 10px; color: #666;">Comprehensive Loan Portfolio Analysis</p>
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th class="col-aging">Total Overdue</th>
            </tr>
        </thead>
        <tbody>
            @forelse($agingData as $row)
                <tr>
                    ...
                    <td class="col-balance text-right">{{ number_format($row['outstanding_balance'], 0) }}</td>
                    ...
                    <td class="col-aging text-right total-overdue">{{ number_format($row['total_overdue'], 0) }}</td>
                </tr>
            @empty
                <tr>
        ...
        <tfoot>
            <tr class="totals-row">
                ...
                <td class="text-right">{{ number_format(collect($agingData)->sum('outstanding_balance'), 0) }}</td>
                <td colspan="4"></td>
                ...
                <td class="text-right total-overdue">{{ number_format(collect($agingData)->sum('total_overdue'), 0) }}</td>
            </tr>
        </tfoot>
    </table>
    ...
    <div class="footer">
        @if($company)
            <p><strong>{{ $company->name }}</strong> - Loan Aging Report</p>
        @endif
        <p>This report was generated automatically on {{ now()->format('Y-m-d H:i:s') }}</p>
        <p>Page 1 of 1 | Confidential Document</p>
    </div>
</body>
</html>
